...
// The loader is the thing under test as much as the module is, so the fixture
// is assembled from the real snippets rather than a copy that can drift from
// what is pasted into Webflow. VER and the staging origin are read out of it
// too: both change at every release, and hardcoding them here would turn a
// routine version bump into a test failure.
...
const stage = loader.match(/var STAG = "([^"]+)"/)[1];
const local = loader.match(/var DEV = "([^"]+)"/)[1];
const version = loader.match(/var VER = "([^"]+)"/)[1];
const [, cdnPrefix, cdnSuffix] = loader.match(/PROD_JS =\s*"([^"]+)" \+ VER \+ "([^"]+)";/);
const release = new URL('.', cdnPrefix + version + cdnSuffix).href;
